...
public class EnlongScript : MonoBehaviour
{
    ...
    //初始化引导arrow
    private GameObject InstantiateEnLong(GameObject enlong)
    {
        if (enlong == null)
        {
            return null;
        ...
        //初始化一个enlong延长线
        GameObject enlong = Instantiate(enlong);

        // Set local variables for the indicator.
        directionIndicatorDefaultRotation = enlong.transform.rotation;
        directionIndicatorRenderer = enlong.GetComponent<Renderer>();
        directionIndicatorChildRenderer = enlong.transform.GetChild(0).GetComponent<Renderer>();
        ...
        foreach (Collider indicatorCollider in enlong.GetComponents<Collider>())
        {
            Destroy(indicatorCollider);
        }

        foreach (Rigidbody rigidBody in enlong.GetComponents<Rigidbody>())
        {
            Destroy(rigidBody);
        }
